...
// 这里的关键是npm run传进来的自定义参数可以通过 process.env.npm_config_config_*获取
...
console.log('webpack_serve: ', process.env.WEBPACK_SERVE);
...
module.exports = {
  ...
  */
  ...
  output: {
    ...
    /**
     * 现在资源文件url直接在根目录，比如127.0.0.1:8080/index.js，这样做缓存控制和cdn不是很方便，
     * 隐刺给资源文件url加一个前缀，比如 /assets/index.js, 
     * webpack-serve也需要修改
     */
    ...
    /**
     * 代码中引用的文件(js/css/图片等)会根据配置合并为一个或多个包，称一个包为chunk
     * 每个chunk包含多个modules，无论是否js，webpack都将引入的文件视为一个module
     * 
     * chunkfilename用来配置个chunk输出的文件名
     * 
     * chunkhash 这个chunk的hash值，文件发生变化时改值也会变，使用chunkhash作为文件名可以纺织浏览器读取旧的缓存文件
     * 
     * 还有一个占位符[id],编译时每个chunk有一个id
     * 这里不适用它，因为id是个递增数字，增加或减少一个chunk，都可能导致其他chunk的id发生变化，导致缓存失效
     */
  ...
  module: {
    ...
    */
    rules: [
...

      {
        ...
        */
        use: {
          ...
          options: {
            /**
             * html-loader接受attrs参数，表示什么标签的什么特性需要调用webpack的loader进行打包，比如img标签的src属性，webpack会把img引用的图片打包，然后src的属性替换为打包后的路径。
             * 使用什么loader代码，同样在module.rules定义中使用匹配的规则
             * 
             * 如果html-loader不指定attrs参数，默认只是img:src, 意味着默认会打包img标签的图片，这里加上link标签的href属性，用来打包入口index.html中引入的favicon.png文件
             * 
             */
      ...
      {
        /**
         * 匹配favicon.png 上面的htmlloader会把入口index.html引用的favicon.png图标文件解析出来进行打包，打包规则就按照这里指定的laoder进行
         */
        test: /favicon\.png$/,
        use: [{
          loader: 'file-loader',
...

      {
        ...
        使用style-loader生成一个内容为最终解析完的css代码的style标签，放到head标签里。
        */
...

      {
        ...
        */
        ...
        /**
         * 图片文件的配置增加一个exclude餐宿
         * 排除favicon.png，因为已经在上面的loader处理了，如果不排除，会被这个loader再处理一遍
         */
        exclude: /favicon\.png$/,
        /*
